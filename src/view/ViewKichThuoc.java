...
public class ViewKichThuoc extends javax.swing.JFrame {
    ...
    private KichThuocService sv = new KichThuocService();
    ...
    void showData(List<KichThuoc> listt) {
        model.setRowCount(0);
        for (KichThuoc kt : listt) {
            model.addRow(new Object[]{kt.getiD_KichThuoc(), kt.getSize()});
        }
    }

    void showDetai(int vt) {
        KichThuoc kt = list.get(vt);
        lbID.setText(kt.getiD_KichThuoc() + "");
        txtTen.setText(kt.getSize());
    }
    ...
    boolean check() {
        ...
        ArrayList<KichThuoc> listsp = (ArrayList<KichThuoc>) sv.getAll();
        int listSize = listsp.size();
        for (int i = 0; i < listSize; i++) {
            if (listsp.get(i).getSize().equalsIgnoreCase(txtTen.getText())) {
                JOptionPane.showMessageDialog(this, "Chat Lieu nay da ton tai");
                return false;
            }
        }
        return true;
    ...
    private KichThuoc getForm() {
        KichThuoc kt = new KichThuoc();
        kt.setSize(txtTen.getText());
        return kt;
    }
    ...
    private void btnThemMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnThemMouseClicked
        // TODO add your handling code here:
        if (check()) {
            KichThuoc kt = getForm();
            boolean add = sv.add(kt);
            if (add == true) {
                list = sv.getAll();
                showData(list);
    ...
    private void btnSuaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnSuaMouseClicked
        ...
        if (row >= 0) {
            ...
            if (check()) {
                KichThuoc kt = getForm();
                boolean update = sv.update(kt, ma);
                if (update == true) {
                    list = sv.getAll();
                    showData(list);
