...
public class ViewChatLieu extends javax.swing.JFrame {
    ...
    private ChatLieuService sv = new ChatLieuService();
    ...
    void showData(List<ChatLieu> listt) {
        model.setRowCount(0);
        for (ChatLieu cl : listt) {
            model.addRow(new Object[]{cl.getId_ChatLieu(), cl.getTenChatLieu()});
        }
    }

    void showDetai(int vt) {
        ChatLieu cl = list.get(vt);
        lbID.setText(cl.getId_ChatLieu() + "");
        txtTen.setText(cl.getTenChatLieu());
    }
    ...
    boolean check() {
        ...
        ArrayList<ChatLieu> listsp = (ArrayList<ChatLieu>) sv.getAll();
        int listSize = listsp.size();
        for (int i = 0; i < listSize; i++) {
            if (listsp.get(i).getTenChatLieu().equalsIgnoreCase(txtTen.getText())) {
    ...
    private void btnSuaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnSuaMouseClicked
        ...
        if (row >= 0) {
            ...
            if (check()) {
                ChatLieu cl = getForm();
                boolean update = sv.update(cl, ma);
                if (update == true) {
                    list = sv.getAll();
                    showData(list);
